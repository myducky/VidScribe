...
class ArticleWriter:
    ...
    def generate(self, text: str, structured_summary: dict, desired_length: int = 1200) -> str:
        ...
        system_prompt = (
            "Write a Chinese WeChat official account article in clean HTML.\n"
            f"{author_persona}\n\n"
        ...
        user_prompt = (
            ...
            "请直接输出 HTML 正文，只允许使用 <h1> <h2> <h3> <p> <ul> <ol> <li> <strong> <em> <blockquote> <br> 标签。\n"
            "不要输出 Markdown，不要输出 ```html 代码块，不要解释。\n"
        ...
        if response.startswith("LLM_FALLBACK::"):
            ...
            return (
                f"<h1>{structured_summary['title_candidates'][0]}</h1>"
                f"<p>{summary}</p>"
                f"<p>这件事真正值得注意的地方在于，{observation}</p>"
                f"<p>很多人会先盯着表层信息，但如果把时间维度稍微拉长一点看，往往会发现另一个细节：{second_point}</p>"
                "<p>这也是我更关心的部分。因为对内容判断来说，重要的通常不是某一个瞬间的热度，而是它背后反映出的结构性变化。"
                "一旦这个变化开始出现，后面的影响往往会沿着产业、用户习惯和资源分配慢慢展开。</p>"
                "<p>所以与其把它理解成一条需要立刻表态的新闻，不如把它当成一个观察窗口。"
                "它未必马上改变一切，但会提醒我们，原来那套默认前提，可能已经在松动。</p>"
                "<p>从写作上说，这类素材最怕两种处理方式：一种是把信息简单复述一遍，另一种是急着下过满的结论。"
                "更稳妥的写法，应该是在事实之上保留一点判断，但不要把判断写得太满。"
                "因为真正有价值的，不是态度有多强，而是能不能把变化的方向说清楚。</p>"
                "<p>这篇内容先整理到这里。它更像是一份阶段性观察，而不是终局结论。"
                "如果后续还有更多信号出现，再往下更新判断会更有意义。</p>"
            )
        return self._normalize_html_response(response)

    @staticmethod
    def _normalize_html_response(response: str) -> str:
        text = response.strip()
