...
VidScribe 是一个基于 FastAPI + Celery 的服务，用来把文本、上传视频，或可解析的视频网页链接整理成公众号文章素材包，并直接输出适合后续对接公众号发布接口的 HTML 正文。
...
- `DELETE /v1/jobs/{job_id}/artifacts`：删除任务媒体缓存或整个任务目录
- `GET /v1/jobs/{job_id}/artifacts/location`：查询任务资源所在本地目录
...
- PostgreSQL
- 可选的 OpenAI 兼容接口
- `yt-dlp` 相关远程视频探测与下载依赖网络和目标平台可访问性
...
- Whisper 侧支持 `WHISPER_INITIAL_PROMPT`，可用于增强品牌名、平台名、术语和数字表达的识别倾向
...
- 中文转写清洗链路会优先使用 LLM 做纠错、断句和标点恢复，失败时再回退到本地规则清洗
...
```bash
curl -X DELETE "http://127.0.0.1:8000/v1/jobs/<job_id>/artifacts?target=media"
```

删除整个任务目录：

```bash
curl -X DELETE "http://127.0.0.1:8000/v1/jobs/<job_id>/artifacts?target=all"
```

### 8. 查询任务资源位置

```bash
curl "http://127.0.0.1:8000/v1/jobs/<job_id>/artifacts/location"
```
...
```json
{
  ...
  "tags": ["短视频", "公众号文章"],
  "article_html": "<h1>标题</h1><p>正文内容</p>",
  "cover": {
    "prompt": "中文封面概念",
    "layout": "居中标题 + 副标题",
    "text_on_cover": "封面短文案"
...
首页工作台支持：
- 直接渲染 HTML 文章正文，而不是显示 HTML 源码
- 复制 HTML、下载 HTML、下载 JSON
- 查看任务资源位置
- 删除下载视频 / 删除任务文件
- 顶部“实时状态”按异步任务 step 显示当前进度，例如“正在转写音频”“正在生成文章”
...
### 2026-03-10
- 文章主输出字段由 `article_markdown` 切换为 `article_html`，前端直接渲染 HTML 阅读效果
- 增加 Whisper 初始提示词和 LLM 转写校正链路，提升中文错词纠正、断句和标点恢复效果
- 增加任务资源管理能力：查看资源位置、删除下载视频、删除任务文件
- 顶部实时状态改为展示异步任务当前步骤

- 新增首页控制台入口 `/`，可直接在浏览器中测试文本分析、远程链接探测、视频上传和异步任务查询
- 新增 `app/prompts/writing_style.md`，把写作风格配置从代码中拆出，支持直接编辑提示词
- 新增 `app/services/prompt_loader.py` 和 `scripts/check_llm.py`，补充提示词加载与 LLM 配置自检能力
- 保持 `/health`、`/v1/health`、`/v1/analyze-text`、异步任务链路和视频探测接口兼容
- 增补首页、LLM 客户端、提示词样式相关测试
...
- 提交前优先运行 `.venv/bin/pytest -q`
- 本地调试远程视频时，优先验证 B 站公开链接；抖音只按尽力解析处理
- 修改提示词优先编辑 [app/prompts/writing_style.md](/Users/ducky/Projects/VidScribe/app/prompts/writing_style.md)，避免把文风逻辑散落到服务代码中
